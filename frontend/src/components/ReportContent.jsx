...
export function FeeStatusTracker({ year, api }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api.reports.feeStatus(month, year, selectedFeeType)
        ...
        {data ? (
          <>
            <Row gutter={16} style={{ marginBottom: 16 }}>
              <Col xs={24} sm={8}>
                <Card size="small" style={{ borderTop: "3px solid #52c41a" }}>
                  <Statistic title="Đã đóng phí" value={data.paid} suffix={`/ ${data.total} thành viên`}
                    styles={{ content: { color: "#52c41a" } }} prefix={<CheckCircleOutlined />} />
                </Card>
              </Col>
              <Col xs={24} sm={8}>
                <Card size="small" style={{ borderTop: "3px solid #ff4d4f" }}>
                  <Statistic title="Chưa đóng phí" value={data.unpaid} suffix={`/ ${data.total} thành viên`}
