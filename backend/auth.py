...
if not SECRET_KEY:
    # Không set SECRET_KEY qua env (vd chạy dev local) → dùng khoá ngẫu nhiên
    # theo mỗi lần khởi động, KHÔNG bao giờ dùng một chuỗi cố định làm dự phòng.
    # Hệ quả: token cũ mất hiệu lực mỗi lần restart — chấp nhận được cho dev,
    # nhưng production PHẢI set SECRET_KEY qua `fly secrets set`.
    print("[auth] CẢNH BÁO: biến môi trường SECRET_KEY chưa được set — dùng khoá ngẫu nhiên tạm thời cho phiên này")
